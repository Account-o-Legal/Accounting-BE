# app/modules/reporting/test_trial_balance_voided_entry.py
...
async def _run() -> None:
    ...
    async with AsyncSession(engine, expire_on_commit=False) as db:
        ...
        trial_balance = await generate_trial_balance(
            db,
            tenant_id=TENANT_ID,
        )

        assert trial_balance["is_balanced"] is True
        assert trial_balance["total_debit"] == 2000
        assert trial_balance["total_credit"] == 2000
        ...
        assert round(accounts_by_code["1000"]["balance"], 2) == 0
        assert round(accounts_by_code["4000"]["balance"], 2) == 0


def test_trial_balance_voided_entry() -> None:
    asyncio.run(_run())


if __name__ == "__main__":
    test_trial_balance_voided_entry()
    print("ok")