...
async def _run():
    ...
    async with AsyncSession(engine, expire_on_commit=False) as db:
        ...
        jan = await generate_trial_balance(
            db,
            tenant_id=TENANT_ID,
        ...
        feb = await generate_trial_balance(
            db,
            tenant_id=TENANT_ID,
        ...
        combined = await generate_trial_balance(
            db,
            tenant_id=TENANT_ID,
            start_date=date(2026, 1, 1),
            end_date=date(2026, 2, 28),
        ...
        assert jan["total_debit"] == 1000
        assert feb["total_debit"] == 1000
        assert combined["is_balanced"] is True


def test_trial_balance_cross_period_void():
    asyncio.run(_run())


if __name__ == "__main__":
    test_trial_balance_cross_period_void()
    print("ok")